using EnumRecords;

namespace CssUI.HTML
{
    /// <summary>
    /// The type attribute controls the data type (and associated control) of an input element.
    /// </summary>
    [EnumRecord<KeywordProperties>]
    public enum EInputType : int
    {/* Docs: https://html.spec.whatwg.org/multipage/input.html#attr-input-type */

        /// <summary>
        /// Arbitrart string
        /// </summary>
        [EnumRecordProperties("hidden")]
        Hidden,

        /// <summary>
        /// Text with no line breaks
        /// </summary>
        [EnumRecordProperties("text")]
        Text,

        /// <summary>
        /// Text with no line breaks
        /// </summary>
        [EnumRecordProperties("search")]
        Search,

        /// <summary>
        /// Text with no line breaks
        /// </summary>
        [EnumRecordProperties("tel")]
        Telephone,

        /// <summary>
        /// An absolute URL
        /// </summary>
        [EnumRecordProperties("url")]
        Url,

        /// <summary>
        /// An e-mail address or list of e-mail addresses
        /// </summary>
        [EnumRecordProperties("email")]
        Email,

        /// <summary>
        /// Text with no line breaks (sensitive information)
        /// </summary>
        [EnumRecordProperties("password")]
        Password,

        /// <summary>
        /// A date (year, month, day) with no time zone
        /// </summary>
        [EnumRecordProperties("date")]
        Date,

        /// <summary>
        /// A date consisting of a year and a month with no time zone
        /// </summary>
        [EnumRecordProperties("month")]
        Month,

        /// <summary>
        /// A date consisting of a week-year number and a week number with no time zone
        /// </summary>
        [EnumRecordProperties("week")]
        Week,

        /// <summary>
        /// A time (hour, minute, seconds, fractional seconds) with no time zone
        /// </summary>
        [EnumRecordProperties("time")]
        Time,

        /// <summary>
        /// A date and time (year, month, day, hour, minute, second, fraction of a second) with no time zone
        /// </summary>
        [EnumRecordProperties("datetime-local")]
        Local,

        /// <summary>
        /// A numerical value
        /// </summary>
        [EnumRecordProperties("number")]
        Number,

        /// <summary>
        /// A numerical value, with the extra semantic that the exact value is not important
        /// </summary>
        [EnumRecordProperties("range")]
        Range,

        /// <summary>
        /// An sRGB color with 8-bit red, green, and blue components
        /// </summary>
        [EnumRecordProperties("color")]
        Color,

        /// <summary>
        /// A set of zero or more values from a predefined list
        /// </summary>
        [EnumRecordProperties("checkbox")]
        Checkbox,

        /// <summary>
        /// An enumerated value
        /// </summary>
        [EnumRecordProperties("radio")]
        Radio,

        /// <summary>
        /// Zero or more files each with a MIME type and optionally a file name
        /// </summary>
        [EnumRecordProperties("file")]
        File,

        /// <summary>
        /// An enumerated value, with the extra semantic that it must be the last value selected and initiates form submission
        /// </summary>
        [EnumRecordProperties("submit")]
        Submit,

        /// <summary>
        /// A coordinate, relative to a particular image's size, with the extra semantic that it must be the last value selected and initiates form submission
        /// </summary>
        [EnumRecordProperties("image")]
        Image,

        /// <summary>
        /// n/a
        /// </summary>
        [EnumRecordProperties("reset")]
        Reset,

        /// <summary>
        /// n/a
        /// </summary>
        [EnumRecordProperties("button")]
        Button,
    }
}

