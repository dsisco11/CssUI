...
namespace CssUI.DOM
{
    ...
    {/* Docs: https://mimesniff.spec.whatwg.org/#understanding-mime-types */
        ...
        {/* Docs:  */
            input = StringCommon.Trim(input, FilterHTTPWhitespace.Instance);
            ...
            Subtype = StringCommon.TrimEnd(Subtype, FilterHTTPWhitespace.Instance);
        ...
        {/* Docs: https://mimesniff.spec.whatwg.org/#sniffing-a-mislabeled-feed */
            ...
            int s = 0;
            var Stream = new DataConsumer<byte>(Header, byte.MinValue);
        ...
        {/* Docs: https://mimesniff.spec.whatwg.org/#signature-for-webm */
            ...
            while (iter < Header.Length && iter < 38)
            {
                if (Header.Slice(iter, 2).Span.SequenceEqual(new byte[] { 0x42, 0x82 }))
                {
                    ...
                    var matched = match_padded_sequence(Array.Empty<byte>(), Header.Slice(iter));
