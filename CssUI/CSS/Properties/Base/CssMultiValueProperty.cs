...
namespace CssUI
{
    ...
    public abstract class CssMultiValueProperty : CssPropertyBase, ICssProperty
    {
        ...
        /// </summary>
        public override bool IsNone { get => (Assigned.FirstOrDefault().Type == ECssValueTypes.NONE); }
        /// <summary>
        /// Return TRUE if the assigned value is set to <see cref="CssValue.Auto"/>
        /// </summary>
        public override bool IsAuto { get => Assigned.FirstOrDefault().Type == ECssValueTypes.AUTO; }
        /// <summary>
        /// Returns TRUE if the assigned value is <see cref="CssValue.Inherit"/>
        /// </summary>
        public override bool IsInherited { get => Assigned.FirstOrDefault().Type == ECssValueTypes.INHERIT; }
        /// <summary>
        ...
        private CssValue Derive_SpecifiedValue(CssValue AssignedValue, out bool Inherited)
        {
            ...
            if (!AssignedValue.IsNull)
            {
                ...
                {
                    case ECssValueTypes.UNSET:// SEE:  https://www.w3.org/TR/css-cascade-4/#valdef-all-unset
        ...
        private void Reinterpret_Computed()
        {
            ...
            {// The only thing that can make the computed value inherit, is being set to the INHERIT value
                if (Specified.First().Type == ECssValueTypes.INHERIT)
                {
                    if (Owner.parentElement is null)
                    {// Root element
