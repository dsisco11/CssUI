...
/// Fragmentation: https://www.w3.org/TR/css-flexbox-1/#pagination
/// </summary>
public class FlexFormattingContext : IFormattingContext
{
    ...
    private class FlexLine
    {
        ...
        /// <summary>
        /// The fragment index where this line starts (for fragmentation).
        /// </summary>
        public int FragmentIndex { get; set; }

        /// <summary>
        /// Whether this line was broken due to fragmentation.
        /// </summary>
        public bool WasFragmented { get; set; }
    }
    ...
    private float CalculateFlexBaseSize(FlexItem item)
    {
        ...
        if (cascaded?.Computed?.IsAuto != false)
        {
    ...
    /// <summary>
    /// Handles fragmentation for flex layout.
    /// Spec: https://www.w3.org/TR/css-flexbox-1/#pagination
    /// </summary>
    private void HandleFragmentation()
    {
        if (_fragmentationContext is null || !IsFragmenting)
        {
            return;
        }

        // For column flex containers, fragmentation occurs along the main axis.
        // For row flex containers, fragmentation occurs along the cross axis (between lines).
        if (IsMainAxisHorizontal)
        {
            HandleCrossAxisFragmentation();
        }
        else
        {
            HandleMainAxisFragmentation();
        }
    }

    /// <summary>
    /// Handles fragmentation along the cross axis (for row flex containers).
    /// Break opportunities occur between flex lines.
    /// </summary>
    private void HandleCrossAxisFragmentation()
    {
        if (_flexLines is null || _fragmentationContext is null)
        {
            return;
        }
        ...
        foreach (var line in _flexLines)
        {
            // Check for forced breaks before the line
            foreach (var item in line.Items)
            {
                var breakResult = _fragmentationContext.EvaluateBreakBefore(item.PrincipalBox);
                if (breakResult.ShouldBreak && breakResult.BreakType == EBreakType.Forced)
                {
                    _fragmentationContext.AdvanceToNextFragmentainer();
                    line.FragmentIndex = _fragmentationContext.FragmentIndex;
                    break;
                }
            }

            // Check if line fits in current fragmentainer
            if (!_fragmentationContext.CanFit(line.CrossSize))
            {
                ...
                if (!avoidBreak)
                {
                    _fragmentationContext.AdvanceToNextFragmentainer();
                    line.FragmentIndex = _fragmentationContext.FragmentIndex;
                    line.WasFragmented = true;
                }
            }

            _fragmentationContext.AdvanceBlockOffset(line.CrossSize + _crossAxisGap);

            // Check for forced breaks after each item in the line
            foreach (var item in line.Items)
            {
                var breakResult = _fragmentationContext.EvaluateBreakAfter(item.PrincipalBox);
                if (breakResult.ShouldBreak && breakResult.BreakType == EBreakType.Forced)
                {
                    _fragmentationContext.AdvanceToNextFragmentainer();
                    break;
    ...
    /// <summary>
    /// Handles fragmentation along the main axis (for column flex containers).
    /// Break opportunities occur between flex items within each line.
    /// </summary>
    private void HandleMainAxisFragmentation()
    {
        if (_flexLines is null || _fragmentationContext is null)
        {
            return;
        ...
        foreach (var line in _flexLines)
        {
            foreach (var item in line.Items)
            {
                // Check for forced breaks before the item
                ...
                // Check if item fits in current fragmentainer
                float itemMainSize = item.TargetMainSize;
                if (!_fragmentationContext.CanFit(itemMainSize))
                {
                    bool avoidBreak = _fragmentationContext.ShouldAvoidBreakInside(item.PrincipalBox);
                    if (!avoidBreak)
                    {
                        _fragmentationContext.AdvanceToNextFragmentainer();
                        item.FragmentIndex = _fragmentationContext.FragmentIndex;
                ...
                _fragmentationContext.AdvanceBlockOffset(itemMainSize + _mainAxisGap);

                // Check for forced breaks after the item
