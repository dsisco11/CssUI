...
namespace CssUI
{
    ...
    {
        #region Properties
        protected eBlock layoutBlock;
        protected ePos layoutPos;
        /// <summary>
        ...
        {
            Lines.Add(new LineBox(Line, new eSize(layoutPos.X, lineHeight)));
            Line = new List<LineBox_Element>(1);
            // Update the width of our layout area
            layoutBlock.Width = Math.Max(layoutBlock.Width, layoutPos.X);
            // Update the height of our layout area
            layoutBlock.Height = Math.Max(layoutBlock.Height, layoutPos.Y + lineHeight);
        ...
        protected void Finalize_Positions()
        {
            ...
            foreach (LineBox line in Lines)
            {
                ...
                {
                    item.Element.Style.Set_Layout_Pos(item.Pos.X, item.Pos.Y);
                }
            }
        }
