    ...
    public partial class CssBox
    {
                ...
                case EBoxDisplayGroup.INLINE:
                    {
                        ...
                        if (this.IsReplacedElement)
                        {
                            ...
                            {
                                Width = CssValue.From_Length(300, EStyleUnit.PX);
                            }
                        }
                    }
                    break;
        ...
        private int Get_Height_For_Block_Formatting_Context()
        {
            ...
            if (!this.HasBlockLevelChildren)
            {
                ...
                {
                    var c = this[i];
                    if (c.OuterDisplayType == EOuterDisplayType.Inline)
                    {
                        topEdge = c.Margin.Top;
                        break;
                    }
                }
                // find our last block-level element and its top-margin-edge
                for (int i = this.Count - 1; i >= 0; i--)
                {
                    var c = this[i];
                    if (c.OuterDisplayType == EOuterDisplayType.Inline)
                    {
                        bottomEdge = c.Margin.Bottom;
                        break;
                    }
                }
            ...
            else
            {
                ...
                {
                    var c = this[i];
                    if (c.OuterDisplayType == EOuterDisplayType.Block)
                    {
                        topEdge = c.Margin.Top;
                        break;
                    }
