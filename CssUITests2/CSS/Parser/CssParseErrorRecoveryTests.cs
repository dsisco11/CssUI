...
public class CssParseErrorRecoveryTests
{
    ...
    #region Graceful Error Recovery Tests (Entry Point Methods)

    [Fact]
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_Rule_UnexpectedEof_ReturnsNull()
    {
        // Arrange - empty input (unexpected EOF)
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("".AsSpan(), reporter);
        ...
        // Assert - should return null and report error
        Assert.Null(result);
        Assert.True(reporter.HasErrors);
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_Rule_ExtraContent_StillReturnsRule()
    {
        // Arrange - valid rule followed by extra content
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("body { color: red; } extra".AsSpan(), reporter);

        // Act
        var result = parser.Parse_Rule();

        // Assert - should return the parsed rule but report extra content error
        Assert.NotNull(result);
        Assert.True(reporter.HasErrors);
        Assert.Contains(reporter.Errors, e => e.ErrorType == ECssParseErrorType.ExtraContentAfterRule);
    }

    [Fact]
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_Decleration_ExpectingIdent_ReturnsNull()
    {
        // Arrange - starts with non-ident (number)
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("123: red".AsSpan(), reporter);

        // Act
        var result = parser.Parse_Decleration();

        // Assert - should return null and report error
        Assert.Null(result);
        Assert.True(reporter.HasErrors);
        Assert.Contains(reporter.Errors, e => e.ErrorType == ECssParseErrorType.ExpectedIdentifier);
    }

    [Fact]
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_ComponentValue_UnexpectedEof_ReturnsNull()
    {
        // Arrange - empty input
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("".AsSpan(), reporter);
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_ComponentValue_ExtraContent_StillReturnsValue()
    {
        // Arrange - valid value followed by extra content
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("red blue".AsSpan(), reporter);

        // Act
        var result = parser.Parse_ComponentValue();

        // Assert - should return the first value but report extra content error
        Assert.NotNull(result);
        Assert.True(reporter.HasErrors);
        Assert.Contains(reporter.Errors, e => e.ErrorType == ECssParseErrorType.ExtraContentAfterRule);
    }
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Consume_CssValue_BadUrl_ReturnsNull()
    {
        // Arrange - bad URL token
        // Note: This requires a bad-url token which is generated by the tokenizer
        // for malformed URLs. We test indirectly via the parser.
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("body { background: url(bad url with spaces); }".AsSpan(), reporter);

        // Act
        var result = parser.Parse_Stylesheet();

        // Assert - should parse but may report errors for invalid URL
        // The exact behavior depends on tokenizer handling
        Assert.NotNull(result);
    }
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Parse_CssValue_InvalidHexColor_ReportsError()
    {
        // Arrange - invalid hex color (invalid hex chars like 'G')
        // The tokenizer still produces a HashToken, but CssHexColorParser.TryParse fails
        var reporter = new CssParseErrorReporter();
        // Note: #GGG gets tokenized as a hash token, which is then parsed as CSS value
        // The error is reported when the hex color parsing fails
        var parser = new CssParser("#GGG".AsSpan(), reporter);

        // Act - parse as a CssValue
        var value = parser.Parse_CssValue();

        // Assert - should return Null value and report error
        Assert.True(value == CssValue.Null || !value.HasValue);
        Assert.True(reporter.HasErrors);
        Assert.Contains(reporter.Errors, e => e.ErrorType == ECssParseErrorType.InvalidHexColor);
    }

    [Fact]
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Valid_Declaration_NoErrors()
    {
        // Arrange - valid declaration
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("color: red".AsSpan(), reporter);

        // Act
        var result = parser.Parse_Decleration();

        // Assert - should parse successfully with no errors
        Assert.NotNull(result);
        Assert.False(reporter.HasErrors);
    }
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void Valid_Rule_NoErrors()
    {
        // Arrange - valid rule
        var reporter = new CssParseErrorReporter();
        var parser = new CssParser("body { color: red; }".AsSpan(), reporter);
    ...
    [Trait("Category", "Parser")]
    [Trait("Category", "ErrorRecovery")]
    public void ECssParseErrorType_HasNewEntryPointErrorTypes()
    {
        // Assert new error types for entry point methods exist
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.UnexpectedEof));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.ExtraContentAfterRule));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.ExpectedIdentifier));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.InvalidDeclaration));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.InvalidComponentValue));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.InvalidHexColor));
        Assert.True(Enum.IsDefined(typeof(ECssParseErrorType), ECssParseErrorType.UnhandledTokenType));
    }
